...
export class MyEventDetailsComponent implements OnInit, OnDestroy {
  ...
  */
  ...
  ngOnDestroy() {
    ...
    if (this.isEventformSubmited != true) {
      ...
      console.log("delete unwanted files");
      ...
      var response = this.pfService.postFormData(this.serviceUrl + "/deletEditedRacetypeFiles", { data: JSON.stringify(data) });
    ...
    if (x) {
      ...
      this.http.get(this.serviceUrl + "/deleteRegistereduser/" + id)
        .subscribe(function (response) {
          ...
          if (response) {
            current.router.navigate(["user/listevent"]);
          }
        }, function (err) {
          current.errorLog = true;
        });
  ...
  */
  ...
  saveRaceClass(formType) {
    ...
    this.uploader.onCompleteItem = (item: any, response: any, status: any, header: any) => {
      ...
      console.log(response);
      this.deleteFiles.push(response);
